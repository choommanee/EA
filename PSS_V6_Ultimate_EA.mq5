   ...
   if(currentLevel < InpMartMaxLevel)
   {
      ...
      if(InpMartUseATR && g_handleAtr != INVALID_HANDLE)
      {
         ...
         if(CopyBuffer(g_handleAtr, 0, 0, 1, atrValues) > 0)
         {
            ...
            double dynamicAtrDist = atrPips * InpMartAtrMultiplier;
      ...
      // Removed Trend Filter block for adding positions to allow Martingale to average out during strong trends.
      ...
      else if(InpMartAllowFallbackAdd && distancePips >= activeFallbackDistance * sessionMult)
      {
         shouldEnter = true;
