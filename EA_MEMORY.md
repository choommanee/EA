...
## Anti-Trap Safety Systems (Added 2026-05-24)

Four new safety systems were added to prevent the "enter wrong direction + get dragged" problem:

### 1. Multi-Timeframe (MTF) Direction Filter

- Uses a higher timeframe EMA (default H1 EMA60) to confirm direction.
- If current TF says BUY but H1 says SELL → no L1 entry, no add-orders.
- Prevents opening baskets against the major trend.
...
New indicator: `g_handleHTFMA`

New function: `GetHTFDirection()`

Modified: `GetKillerMaDirection()` now checks HTF before returning direction.

### 2. Emergency Exit (Hard Safety Net)

- Closes basket immediately if floating loss or distance exceeds absolute limits.
- Does NOT require ADX/MA/trend confirmation — pure loss/distance check.
- Acts as the last line of defense before margin call.

New inputs:

- `InpUseEmergencyExit=true`
- `InpEmergencyLossPct=5.0`
- `InpEmergencyDistPips=200`

New function: `CheckEmergencyExit()`

Debug tag: `PSSDBG|EMERGENCY`

### 3. Basket Age Guard

- Stops adding orders after basket has been open for `InpStopAddAfterHours` (default 12h).
- Force-closes the entire basket after `InpMaxBasketAgeHours` (default 24h).
- Prevents indefinitely-held losing baskets.

New inputs:

- `InpUseBasketAge=true`
- `InpMaxBasketAgeHours=24`
- `InpStopAddAfterHours=12`

New functions: `GetBasketOpenTime()`, `CheckBasketAgeGuard()`

Debug tag: `PSSDBG|AGE`

### 4. Opposite Momentum Block

- Blocks add-orders if the last closed candle is a strong candle against basket direction.
- For example: basket is BUY but a 30-pip bearish candle just closed → no add.
- Prevents adding into a violent reversal.

New inputs:

- `InpBlockOnStrongOpposite=true`
- `InpStrongCandlePips=30`

New function: `CheckOppositeMomentumBlock()`

Debug tag: `PSSDBG|MOMENTUM`

### 5. Runaway Guard Changed to Scoring System

Old logic required ALL conditions: `levelHit AND trendAgainst AND (distanceHit OR lossHit)`.

New scoring system:

- `trendAgainst` = 2 points
- `distanceHit` = 1 point
- `lossHit` = 1 point
- `levelHit` = 1 point
- Total score >= 3 → runaway active

This means runaway can trigger even without ADX confirmation if distance + loss + level are all hit (score = 3).

### Execution Order in RunSmcMartingale()

1. Profit close check (most important)
2. Risk guard (daily loss/drawdown)
3. **Emergency Exit** (hard safety net)
4. **Basket Age Guard** (force close if too old)
5. Time window check
6. New basket or add-order logic:
   - Runaway guard
   - **MTF Filter** (block add if HTF disagrees)
   - **Age stop-add** (stop add if basket too old)
   - **Opposite Momentum Block** (stop add on strong opposing candle)
   - Distance check, gate check, lot check
   - Open order

### Set File Updated

`.set` file updated with all new inputs and corrected values:

- `InpUseRunawayGuard=true` (was false in old set)
- `InpMaxDrawdownPercent=12.0` (was 0/off in old set)
- `InpDailyProfitTarget=100.0` (was 100000 in old set)
- `InpDailyLossLimit=1000.0` (was 10000 in old set)
- `InpMaxLevel=18` (was 30 in old set)
- All new safety inputs added with defaults.
