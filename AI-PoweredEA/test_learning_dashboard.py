"""
Test suite for Learning Dashboard and Reporting System
Tests dashboard functionality, chart generation, and reporting
"""
...
from Python.learning_dashboard import (
...
class TestLearningDashboard(unittest.TestCase):
    ...
    def test_generate_system_health_report(self):
        ...
        print("\n--- Testing System Health Report Generation ---")
        
        report = self.dashboard.generate_system_health_report(24)
        
        self.assertIsInstance(report, ReportData)
        self.assertEqual(report.report_type, ReportType.RESOURCE_USAGE)
        self.assertIn("System Health", report.title)
        self.assertIsInstance(report.charts, list)
        self.assertIsInstance(report.tables, list)
        
        print("✓ Generated system health report")
    ...
    def test_metric_time_series_generation(self):
        """Test metric time series data generation"""
        print("\n--- Testing Metric Time Series Generation ---")
        
        start_time = datetime.now() - timedelta(hours=24)
        end_time = datetime.now()
        
        time_series = self.dashboard._get_metric_time_series(
            "test_model", "accuracy", start_time, end_time
        )
        ...
        if time_series:
            point = time_series[0]
            self.assertIn('x', point)  # timestamp
            self.assertIn('y', point)  # value
    ...
    def test_widget_data_generation(self):
        ...
        for widget_type, config in widget_types:
            ...
            self.assertNotIn('error', widget_data)  # Should not have errors
            
        print(f"✓ Generated data for {len(widget_types)} widget types")
    ...
    def test_table_generation(self):
        ...
        metrics_table = self.dashboard._create_metrics_summary_table(
            "test_model", start_time, end_time
        )
        
        self.assertIsInstance(metrics_table, dict)
        self.assertIn('title', metrics_table)
        self.assertIn('headers', metrics_table)
        self.assertIn('rows', metrics_table)
        
        # Test model comparison table
        comparison_table = self.dashboard._create_model_comparison_table(
            ['model_1', 'model_2']
        )
        
        self.assertIsInstance(comparison_table, dict)
        self.assertIn('headers', comparison_table)
        self.assertIn('rows', comparison_table)
    ...
    def test_dashboard_configuration(self):
        ...
        config = self.dashboard.dashboard_config
        self.assertIn('refresh_interval', config)
        self.assertIn('theme', config)
        self.assertIn('auto_refresh', config)
        
        # Test dashboard summary
        summary = self.dashboard._get_dashboard_summary()
        self.assertIsInstance(summary, dict)
        self.assertIn('active_widgets', summary)
        self.assertIn('last_updated', summary)
        self.assertIn('system_status', summary)
        
        print("✓ Dashboard configuration validation passed")
...
if __name__ == '__main__':
    print("="*60)
    print("LEARNING DASHBOARD TESTS")
    print("="*60)
