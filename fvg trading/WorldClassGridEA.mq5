...
//|                   World-Class Grid Trading System with 1:3 R:R  |
//|                     Based on Top Performing EAs Analysis        |
//+------------------------------------------------------------------+
#property copyright "World Class Grid EA"
#property version   "3.00"
#property description "Professional Grid System - Target 1:3 Risk:Reward + 75%+ Win Rate"
...
//+------------------------------------------------------------------+
//| Input Parameters - Based on World's Best Grid EAs              |
//+------------------------------------------------------------------+
input group "=== Professional Grid Settings ==="
input int InpMagicNumber = 777777;                   // Magic Number
input double InpFixedLotSize = 0.01;                 // Fixed Lot Size (0 = auto calculation)
input double InpRiskPercent = 0.5;                   // Risk Per Trade (% of balance) - ลดลง
input double InpTargetRiskReward = 2.0;              // Target Risk:Reward Ratio (1:2) - ปรับลง
input int InpGridStepPips = 50;                      // Grid Step (pips) - เพิ่มขึ้นเพื่อลด lot
input int InpMaxGridLevels = 3;                      // Max Grid Levels - ลดลงเพื่อควบคุม

input group "=== Professional Risk Management ==="
input double InpMaxDrawdownPercent = 10.0;           // Max Drawdown (% of balance) - เข้มงวดขึ้น
input double InpDailyLossLimit = 3.0;               // Daily Loss Limit (% of balance) - เข้มงวดขึ้น
input bool InpUseHardStopLoss = true;               // Use Hard Stop Loss
input double InpStopLossMultiplier = 1.5;           // Stop Loss = Grid Range × Multiplier - ลดลง
input double InpMinFreeMarginPercent = 30.0;         // Min Free Margin (% of balance)

input group "=== Profit Optimization ==="
input double InpMinProfitPerLot = 3.0;              // Minimum Profit Per Lot ($) - เพิ่มจาก $0.04
...
int OnInit()
{
    Print("=== 🌟 WORLD CLASS GRID SYSTEM STARTED 🌟 ===");
...
void OnTick()
{
    ...
    // 🎯 Check for profit target (1:3 Risk:Reward)
    if(grid_count > 0)
    {
...
void UpdateMarketAnalysis()
{
    ...
    // Determine if market is ranging (price near EMA = ranging)
    double distance_from_ema = MathAbs(current_price - current_trend_value);
    double atr_equivalent = InpGridStepPips * GetPipValue() * 3; // 3x grid step as range threshold
...
bool ShouldStartNewGrid()
{
    ...
    // Time filter (avoid news times)
    MqlDateTime time_struct;
    TimeToStruct(TimeCurrent(), time_struct);

    // Avoid major news hours (can be expanded)
...
void StartNewGrid()
{
    ...
    if(InpUseTrendFilter)
    {
        ...
        {
            direction = "SELL"; // Price above EMA, expect reversion down
            entry_price = current_bid;
        }
        else
        {
            direction = "BUY"; // Price below EMA, expect reversion up
...
//+------------------------------------------------------------------+
//| Calculate Position Size (Professional Risk Management)          |
//+------------------------------------------------------------------+
double CalculatePositionSize()
{
    ...
    // 🛡️ Safety checks
    if(equity < balance * 0.8) // If equity < 80% of balance
    {
        Print("⚠️ Low equity detected: ", equity, " vs balance: ", balance);
    ...
    // Calculate for Gold/XAU specifically
    double lot_size = 0.01; // Default safe size

    if(StringFind(_Symbol, "XAU") >= 0 || StringFind(_Symbol, "GOLD") >= 0)
    {
        // Gold: More conservative calculation
        lot_size = risk_amount / 1000.0; // $1000 risk per lot for Gold
    }
    else
    {
        // Forex pairs
        double pip_value = GetPipValue();
        double sl_distance_pips = InpGridStepPips * InpMaxGridLevels;
        lot_size = risk_amount / (sl_distance_pips * pip_value * 10);
    }

    // Normalize and safety limits
    lot_size = NormalizeLotSize(lot_size);

    // Maximum lot size based on free margin
    double max_lot_by_margin = free_margin / 1000.0; // Conservative margin calculation
    if(lot_size > max_lot_by_margin)
    {
        lot_size = max_lot_by_margin;
        Print("📉 Lot size reduced due to margin: ", lot_size);
    }

    // Ensure minimum
    if(lot_size < symbolInfo.LotsMin())
        lot_size = symbolInfo.LotsMin();

    // Ensure maximum
    if(lot_size > symbolInfo.LotsMax())
        lot_size = symbolInfo.LotsMax();

    Print("📊 Calculated lot size: ", lot_size, " | Free margin: $", free_margin);
    return lot_size;
}
...
//| Open Grid Position                                               |
//+------------------------------------------------------------------+
bool OpenGridPosition(string direction, double price, double lot_size, int level)
{
    if(grid_count >= 20) return false; // Array limit
    ...
    Print("📤 Attempting to open: ", direction, " ", lot_size, " lots");
    ...
    if(success)
    {
        ...
        grid_positions[grid_count].lots = lot_size;
    ...
    else
    {
        ...
        Print("📊 Error details - Lot: ", lot_size, " | Price: ", price, " | Free Margin: $", free_margin);
        return false;
    }
}
...
//+------------------------------------------------------------------+
//| Calculate Target Profit (1:3 Risk:Reward)                      |
//+------------------------------------------------------------------+
double CalculateTargetProfit()
{
    double total_risk = CalculateStopLoss();
    return MathAbs(total_risk) * InpTargetRiskReward; // 1:3 Risk:Reward
}
...
{
    double balance = account.Balance();
    return -(balance * (InpRiskPercent / 100.0)); // Max risk per grid cycle
}
...
void RecordTradeResult(double profit, string result)
{
    ...
    // Calculate profit in pips for statistics
    double pip_value = GetPipValue();
    double total_lots = GetTotalGridLots();
    if(total_lots > 0)
    {
        double profit_pips = profit / (total_lots * pip_value * 10);
...
void UpdatePerformanceDisplay()
{
    ...
    string chart_comment = StringFormat(
        ...
        "🛡️ Drawdown: %.2f%% | Margin Used: %.1f%%\n" +
        "🔄 Grid: %s (%d levels) | Fixed Lot: %.2f\n" +
        "📊 Market: %s | Spread: %.1f pips\n" +
        "💡 Safety: 10x margin | Risk: %.1f%% per trade",
