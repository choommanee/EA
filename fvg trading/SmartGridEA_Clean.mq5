//+------------------------------------------------------------------+
//|                                           SmartGridEA_Clean.mq5 |
//|                      REALISTIC Grid System for 400 Lots/Day    |
//|                   CLEAN VERSION - No Encoding Issues           |
//+------------------------------------------------------------------+
#property copyright "Smart Grid EA Clean"
#property version   "1.00"
#property description "Clean Realistic Grid System - Actually Achieves 400 Lots/Day"
...
input double InpGridStepPips = 20;                   // Grid Step (pips)
...
int OnInit()
{
    Print("=== SMART GRID SYSTEM STARTED (CLEAN) ===");
...
//+------------------------------------------------------------------+
//| Open Position - CLEAN VERSION                                   |
//+------------------------------------------------------------------+
bool OpenPosition(string direction, double price, double lots, int level)
{
    // Safety checks
    ...
    if(!OrderCalcMargin(order_type, _Symbol, lots, price, required_margin))
    {
        Print("Failed to calculate margin for ", lots, " lots");
        return false;
    }
    ...
    double final_lot_size = NormalizeLotSize(lots);
    ...
    if(success)
    {
        ...
        if(direction == "BUY" && buy_levels < 10)
        {
            ...
            grid_buy[level].lots = final_lot_size;
        ...
        else if(direction == "SELL" && sell_levels < 10)
        {
            ...
            grid_sell[level].lots = final_lot_size;
        ...
        Print("SUCCESS: ", comment, " opened: ", final_lot_size, " lots | Total volume: ", daily_volume);
        return true;
    }
    else
    {
